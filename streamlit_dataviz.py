...
sns.heatmap(pd.crosstab(data.major_discipline, data.education_level, normalize=True, margins = True), cmap=cont_colors, annot=True, annot_kws={"fontsize":9}, ax=ax, fmt=".1%", vmin = 0.01, vmax = 0.25)
...
sns.heatmap(pd.crosstab(data.company_size, data.company_type, normalize = True, margins=True), cmap=cont_colors, annot = True, annot_kws={"fontsize":9}, fmt=".1%", ax = ax, vmin = 0.01, vmax = 0.25)
